...
test("POST /api/settings/discover without query updates account id and sweeps spaces", async () => {
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      JIRA_API_TOKEN: "tok",
      JIRA_ACCOUNT_ID: "",
      JIRA_SPACES: { ABC: { teamId: "t1", fields: { team: "old" } } },
    }),
  );
  ...
  try {
    ...
    assert.equal(res.json.spaces.ABC.fields.team, "team-ABC");
    const onDisk = JSON.parse(fs.readFileSync(path.join(tmpDir, "config.json"), "utf8"));
    assert.equal(onDisk.JIRA_ACCOUNT_ID, "acct-1");
    assert.equal(onDisk.JIRA_SPACES.ABC.fields.team, "team-ABC");
    // teamId is preserved across discovery (user-set, not overwritten by empty discovery)
    assert.equal(onDisk.JIRA_SPACES.ABC.teamId, "t1");
  } finally {
...
test("POST /api/settings/discover?space=KEY only touches that space", async () => {
  ...
  try {
    ...
    assert.equal(res.json.spaces.XYZ.fields.sprint, "customfield_10020");
...
test("POST /api/settings/discover returns per-space error without aborting", async () => {
  ...
  try {
    ...
    assert.ok(!res.json.spaces.ABC.error);
  } finally {
    discoveryMock.discoverAccountId = origAcct;
    discoveryMock.discoverSpaceFields = origFields;
  }
...
test("PUT /api/settings/spaces/:key updates teamId and preserves fields", async () => {
  ...
  const onDisk = JSON.parse(fs.readFileSync(path.join(tmpDir, "config.json"), "utf8"));
  assert.equal(onDisk.JIRA_SPACES.ABC.teamId, "new-team-uuid");
  assert.equal(onDisk.JIRA_SPACES.ABC.fields.team, "customfield_10001");
});
...
test("PUT /api/settings/spaces/:key rejects non-string teamId", async () => {
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      JIRA_API_TOKEN: "tok",
      JIRA_ACCOUNT_ID: "",
      JIRA_SPACES: { ABC: { teamId: "", fields: {} } },
    }),
  );
  const app = makeApp();
  const res = await call(app, "PUT", "/api/settings/spaces/ABC", { teamId: 1234 });
  assert.equal(res.status, 400);
});
...
test("PUT /api/settings/spaces/:key clears prior error field", async () => {
  ...
  const app = makeApp();
  const res = await call(app, "PUT", "/api/settings/spaces/ABC", { teamId: "fresh" });
  assert.equal(res.status, 200);
