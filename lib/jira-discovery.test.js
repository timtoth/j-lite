      ...
      issuetypes: [
        {
          ...
          fields: {
            ...
            customfield_12037: { fieldId: "customfield_12037", name: "Product", allowedValues: [] },
...
test("discoverSpaceFields maps known field names to ids", async () => {
  ...
  assert.deepEqual(result, {
    ...
    customFields: {
      product: { fieldId: "customfield_12037", allowedValues: [] },
    },
  });
});
...
test("discoverSpaceFields handles missing fields gracefully", async () => {
  ...
  const result = await discoverSpaceFields(jiraRequest, "XYZ");
  assert.equal(result.fields.team, "");
  assert.equal(result.fields.sprint, "");
  assert.equal(result.fields.product, undefined);
...
test("discoverSpaceFields builds customFields map from select-list allowedValues", async () => {
  const jiraRequest = fakeJiraRequest({
    "GET /rest/api/3/issue/createmeta": () => ({
      projects: [
        {
          ...
          issuetypes: [
            {
              ...
              fields: {
                ...
                customfield_99: {
                  ...
                  name: "Free text",
                  // no allowedValues → not a select-list, must be excluded
                },
              },
            },
          ],
...
test("discoverSpaceFields no longer populates fields.product", async () => {
  const jiraRequest = fakeJiraRequest({
    "GET /rest/api/3/issue/createmeta": () => ({
      projects: [
        {
...
test("discoverSpaceFields excludes builtins from customFields even when they expose allowedValues", async () => {
  const jiraRequest = fakeJiraRequest({
    "GET /rest/api/3/issue/createmeta": () => ({
      projects: [
        {
          ...
          issuetypes: [
            {
              ...
              fields: {
                customfield_10020: {
                  ...
                  name: "Sprint",
                  schema: { type: "array", custom: "com.pyxis.greenhopper.jira:gh-sprint", customId: 10020 },
                  allowedValues: [{ value: "S1" }, { value: "S2" }],
                },
                customfield_30000: {
  ...
  const result = await discoverSpaceFields(jiraRequest, "ABC");
  assert.equal(result.fields.sprint, "customfield_10020");
  // Sprint must not also appear in customFields, even though it exposes allowedValues.
  assert.equal(result.customFields?.sprint, undefined);
  assert.deepEqual(result.customFields, {
    region: { fieldId: "customfield_30000", allowedValues: ["NA"] },
  });
});

test("discoverSpaceFields propagates required:true on customFields", async () => {
  const jiraRequest = fakeJiraRequest({
    "GET /rest/api/3/issue/createmeta": () => ({
      projects: [
        {
  ...
  const result = await discoverSpaceFields(jiraRequest, "ABC");
  assert.equal(result.customFields.product.required, true);
  assert.equal(result.customFields.region.required, undefined);
});

test("discoverSpaceFields first-wins on customField name collision", async () => {
  const jiraRequest = fakeJiraRequest({
    "GET /rest/api/3/issue/createmeta": () => ({
      projects: [
        {
  ...
  const result = await discoverSpaceFields(jiraRequest, "ABC");
  assert.deepEqual(result.customFields, {
    product: { fieldId: "customfield_100", allowedValues: ["Alpha"] },
  });
});
