...
const CREATE_TOOL = {
  ...
  description:
    ...
    "inferred from the parent epic key (RL, CUS, etc.). Field IDs are " +
    "looked up from per-space configuration; missing fields are " +
    "auto-discovered on first use and on required-field failures.",
  inputSchema: {
    type: "object",
    properties: {
      summary:         { type: "string", description: "Ticket title." },
      description:     { type: "string", description: "Plain-text body." },
      parent_epic_key: { type: "string", description: "Existing epic key, e.g. RL-456." },
      issue_type:      { type: "string", enum: ["Story", "Task", "Bug"] },
      product:         { type: "string", enum: ["Rightsline", "Alliant"] },
    },
...
const DISCOVER_TOOL = {
  ...
  inputSchema: {
    type: "object",
    properties: {
      space_key: { type: "string", description: "Project key, e.g. RL, CUS, PSO." },
    },
    required: ["space_key"],
  },
};
